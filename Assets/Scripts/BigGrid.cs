        ...
        for (int i = 0; i < Data._bigHigh; i++)
        {
            for (int j = 0; j < Data._bigWidth; j++)
            {
                ...
                if (!Data.Blank)
                {
                    if (Data.Platform)
                    {
                        if (i == Data._bigHigh - 1)
                        {
                            _instatitile.GetComponent<OurGrid>()._gridType = 1;
                        }
                        else
                        {
                            _instatitile.GetComponent<OurGrid>()._gridType = 0;
                        }
                    }
                    if (Data.Rpg)
                    {
                        _instatitile.GetComponent<OurGrid>()._gridType = 2;
                    }
                }
                else
                {
